...
function checkPages(value: string, book: ShelfBook) {
  ...
  if (!Number.isInteger(pages) || pages <= 0) return 'Enter a whole number of pages'
...
export function ReadingCard({ book, update }: Props) {
  ...
  const percent = book.pageCount ? Math.round((book.pagesRead / book.pageCount) * 100) : 0
  ...
  return (
    <div className="flex gap-5 rounded-xl bg-white/15 p-5">
      {book.coverUrl && (
        <img src={book.coverUrl} alt={book.title} className="w-24 rounded object-cover" />
      )}
