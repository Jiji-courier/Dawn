...
function Search() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-8 py-10">
      ...
      {results?.length === 0 && (
        <p className="mt-4">The shelves are quiet on that one. Perhaps it's still being written.</p>
      )}
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
        {results?.map((book) => {
          ...
          return (
            ...
            >
              {book.coverUrl && (
                <img className="w-full max-h-48 object-cover rounded" src={book.coverUrl} alt={book.title} />
              )}
              <h2>{book.title}</h2>
