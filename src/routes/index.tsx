...
function Shelf() {
  const { shelf, loadExample, clear } = useShelf()
      ...
      {SECTIONS.map(({ status, label }) => {
        ...
        return (
          <section key={status}>
            ...
            {books.map((book) => (
              <div key={book.id} className="mb-2 rounded-lg bg-white/10 px-4 py-3">
                <p>{book.title}</p>
                <p className="text-sm text-white/60">{book.author}</p>
                {book.startDate && (
                  <p className="text-sm text-white/60">Starts {formatDate(book.startDate)}</p>
                )}
              </div>
            ))}
          </section>
